...
* \section WISE_project WiSE-Studio project
  ...
  -# Select File, Import, Existing Projects into Workspace
     <tt>C:\\Users\\{username}\\ST\\BlueNRG-LP DK x.x.x\\Projects\\Peripheral_Examples\\Examples_HAL\\TIM\\TIM_PWMInput\\WiSE-Studio\\{STEVAL-IDB011V1}</tt> 
  -# Select desired configuration to build
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
  if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_2)
  {
    ...
    if (uwIC2Value != 0)
    {
      ...
      uwFrequency =  (uint32_t) ( HAL_TIM_GetPeriphClock(htim1.Instance) / uwIC2Value );
    }
    else
    {
      uwDutyCycle = 0;
