...
* \section WISE_project WiSE-Studio project
  ...
  -# Select File, Import, Existing Projects into Workspace
     <tt>C:\\Users\\{username}\\ST\\BlueNRG-LP DK x.x.x\\Projects\\Peripheral_Examples\\Examples_HAL\\RTC\\RTC_Autocalibration\\WiSE-Studio\\{STEVAL-IDB011V1}</tt> 
  -# Select desired configuration to build
...
{  
  ...
  /* Get the Low Speed frequency:  TIMx is used to measure the LSI frequency */
...
static uint32_t GetLSIFrequency(void)
{
  /* Set TIMx instance */
  htim1.Instance = TIMx;
  /* Connect internally the TIMx Input Capture of TIM_CHANNEL_1 to the LSI clock output */
  ...
  /* Wait until the TIMx gets 2 LSI edges */
  while (uwCaptureNumber != 2)
  {
  }
  
  /* Disable TIMx Capture/Compare channel Interrupt Request */
  ...
  if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1)
  {
    ...
    else if(uhCaptureIndex == 1)
    {
      ...
      /* Frequency computation: for this example TIMx (TIMx) is clocked by
         64 MHz Clk */ 
      uwFrequency = HAL_TIM_GetPeriphClock(htim1.Instance) / uwDiffCapture;
      uhCaptureIndex = 0;
      uwCaptureNumber = 2;
    }
  }
