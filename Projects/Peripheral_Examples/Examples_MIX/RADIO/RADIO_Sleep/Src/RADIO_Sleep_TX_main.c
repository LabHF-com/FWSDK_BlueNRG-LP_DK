...
* \section WISE_project WiSE-Studio project
  ...
  -# Select File, Import, Existing Projects into Workspace
     <tt>C:\\Users\\{username}\\ST\\BlueNRG-LP DK x.x.x\\Projects\\Peripheral_Examples\\Examples_MIX\\RADIO\\SleepTx\\WiSE-Studio\\{STEVAL-IDB011V1}</tt> 
  -# Select desired configuration to build
...
int main(void)
{
  ...
  while(1) {
    ...
    if(print_res) {
      ...
      printf("Channel = %d,Timeout errors = %d,CRC errors = %d,PER = %d.%02d  %%\r\n",channel, timeout_error_counter_res, crc_error_counter_res, PRINT_INT((timeout_error_counter_res + crc_error_counter_res)/packet_counter_res*100.0),PRINT_FLOAT((timeout_error_counter_res + crc_error_counter_res)/packet_counter_res*100.0));
