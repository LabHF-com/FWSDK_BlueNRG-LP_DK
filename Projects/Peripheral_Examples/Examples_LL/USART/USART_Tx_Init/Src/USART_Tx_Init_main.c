...
* \section WISE_project WiSE-Studio project
  ...
  -# Select File, Import, Existing Projects into Workspace
     <tt>C:\\Users\\{username}\\ST\\BlueNRG-LP DK x.x.x\\Projects\\Peripheral_Examples\\Examples_LL\\USART\\USART_Tx_Init\\WiSE-Studio\\{STEVAL-IDB011V1}</tt> 
  -# Select desired configuration to build
...
static void MX_USART_UART_Init(void)
{
  ...
  //LL_USART_EnableIT_TXFT(USART1);
  LL_USART_EnableIT_RXNE(USART1);
  LL_USART_EnableIT_ERROR(USART1);

  /* USART1 interrupt Init */
  NVIC_SetPriority(USART1_IRQn, IRQ_MED_PRIORITY);
