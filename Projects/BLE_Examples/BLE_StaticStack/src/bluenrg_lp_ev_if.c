...
typedef void (* aci_l2cap_cos_connection_event_type)(uint16_t Connection_Handle,
                                             uint8_t Event_Type,
                                             uint8_t Identifier,
                                             uint16_t SPSM,
                                             uint16_t Peer_MTU,
                                             ...
                                             conn_cid_t conn_cid[]);
/**
 * @brief Event generated when a L2CAP_CREDIT_BASED_CONNECTION_REQ or
 *        L2CAP_CREDIT_BASED_CONNECTION_RSP  is received from peer device to
 *        create one or more (up to 5) L2CAP Enhanced Credit Based Flow Control
 *        channels.
 * @param Connection_Handle Connection handle that identifies the connection.
 *        Values:
 *        - 0x0000 ... 0x0EFF
...
void aci_l2cap_cos_connection_event(uint16_t Connection_Handle,
                                    ...
                                    conn_cid_t conn_cid[])
{
  if (ev_call_table[ACI_L2CAP_COS_CONNECTION_EVENT] != NULL) {
      aci_l2cap_cos_connection_event_type ptr = (aci_l2cap_cos_connection_event_type)ev_call_table[ACI_L2CAP_COS_CONNECTION_EVENT];
      ptr (Connection_Handle,Event_Type,Identifier,SPSM,Peer_MTU,Peer_MPS,Initial_Credits,Result,CID_Count,conn_cid);
  }
}
typedef void (* aci_l2cap_ecfc_reconfiguration_event_type)(uint16_t Connection_Handle,
                                                   uint8_t Event_Type,
                                                   uint16_t Result,
                                                   uint8_t Identifier,
                                                   uint16_t Peer_MTU,
...
void aci_l2cap_ecfc_reconfiguration_event(uint16_t Connection_Handle,
                                          ...
                                          local_reconf_cid_t local_reconf_cid[])
{
  if (ev_call_table[ACI_L2CAP_ECFC_RECONFIGURATION_EVENT] != NULL) {
      aci_l2cap_ecfc_reconfiguration_event_type ptr = (aci_l2cap_ecfc_reconfiguration_event_type)ev_call_table[ACI_L2CAP_ECFC_RECONFIGURATION_EVENT];
      ptr (Connection_Handle,Event_Type,Result,Identifier,Peer_MTU,Peer_MPS,CID_Count,local_reconf_cid);
  }
}
...
typedef void (* aci_att_clt_read_multiple_var_len_resp_event_type)(uint16_t Connection_Handle,
                                                           uint16_t Event_Data_Length,
                                                           uint8_t Set_Of_Values[]);
/**
...
void aci_att_clt_read_multiple_var_len_resp_event(uint16_t Connection_Handle,
                                                  ...
                                                  uint8_t Set_Of_Values[])
{
  if (ev_call_table[ACI_ATT_CLT_READ_MULTIPLE_VAR_LEN_RESP_EVENT] != NULL) {
      aci_att_clt_read_multiple_var_len_resp_event_type ptr = (aci_att_clt_read_multiple_var_len_resp_event_type)ev_call_table[ACI_ATT_CLT_READ_MULTIPLE_VAR_LEN_RESP_EVENT];
      ptr (Connection_Handle,Event_Data_Length,Set_Of_Values);
  }
}
typedef void (* aci_gatt_eatt_srv_attribute_modified_event_type)(uint16_t Connection_Handle,
                                                         uint16_t CID,
                                                         uint16_t Attr_Handle,
                                                         uint16_t Attr_Data_Length,
                                                         uint8_t Attr_Data[]);
...
void aci_gatt_eatt_srv_attribute_modified_event(uint16_t Connection_Handle,
                                                ...
                                                uint8_t Attr_Data[])
{
  if (ev_call_table[ACI_GATT_EATT_SRV_ATTRIBUTE_MODIFIED_EVENT] != NULL) {
      aci_gatt_eatt_srv_attribute_modified_event_type ptr = (aci_gatt_eatt_srv_attribute_modified_event_type)ev_call_table[ACI_GATT_EATT_SRV_ATTRIBUTE_MODIFIED_EVENT];
      ptr (Connection_Handle,CID,Attr_Handle,Attr_Data_Length,Attr_Data);
  }
}
typedef void (* aci_gatt_eatt_proc_timeout_event_type)(uint16_t Connection_Handle,
                                               uint16_t CID);
/**
 * @brief This event is generated by the client/server to the application on a
 *        GATT timeout (30 seconds). This is a critical event that should not
...
void aci_gatt_eatt_proc_timeout_event(uint16_t Connection_Handle,
                                      uint16_t CID)
{
  if (ev_call_table[ACI_GATT_EATT_PROC_TIMEOUT_EVENT] != NULL) {
      aci_gatt_eatt_proc_timeout_event_type ptr = (aci_gatt_eatt_proc_timeout_event_type)ev_call_table[ACI_GATT_EATT_PROC_TIMEOUT_EVENT];
      ptr (Connection_Handle,CID);
  }
}
typedef void (* aci_eatt_clt_find_info_resp_event_type)(uint16_t Connection_Handle,
                                                uint16_t CID,
                                                uint8_t Format,
                                                uint16_t Event_Data_Length,
                                                uint8_t Handle_UUID_Pair[]);
...
void aci_eatt_clt_find_info_resp_event(uint16_t Connection_Handle,
                                       ...
                                       uint8_t Handle_UUID_Pair[])
{
  if (ev_call_table[ACI_EATT_CLT_FIND_INFO_RESP_EVENT] != NULL) {
      aci_eatt_clt_find_info_resp_event_type ptr = (aci_eatt_clt_find_info_resp_event_type)ev_call_table[ACI_EATT_CLT_FIND_INFO_RESP_EVENT];
      ptr (Connection_Handle,CID,Format,Event_Data_Length,Handle_UUID_Pair);
  }
}
typedef void (* aci_eatt_clt_find_by_type_value_resp_event_type)(uint16_t Connection_Handle,
                                                         uint16_t CID,
                                                         uint8_t Num_of_Handle_Pair,
                                                         Attribute_Group_Handle_Pair_t Attribute_Group_Handle_Pair[]);
/**
...
void aci_eatt_clt_find_by_type_value_resp_event(uint16_t Connection_Handle,
                                                ...
                                                Attribute_Group_Handle_Pair_t Attribute_Group_Handle_Pair[])
{
  if (ev_call_table[ACI_EATT_CLT_FIND_BY_TYPE_VALUE_RESP_EVENT] != NULL) {
      aci_eatt_clt_find_by_type_value_resp_event_type ptr = (aci_eatt_clt_find_by_type_value_resp_event_type)ev_call_table[ACI_EATT_CLT_FIND_BY_TYPE_VALUE_RESP_EVENT];
      ptr (Connection_Handle,CID,Num_of_Handle_Pair,Attribute_Group_Handle_Pair);
  }
}
typedef void (* aci_eatt_clt_read_by_type_resp_event_type)(uint16_t Connection_Handle,
                                                   uint16_t CID,
                                                   uint8_t Handle_Value_Pair_Length,
                                                   uint16_t Data_Length,
                                                   uint8_t Handle_Value_Pair_Data[]);
...
void aci_eatt_clt_read_by_type_resp_event(uint16_t Connection_Handle,
                                          ...
                                          uint8_t Handle_Value_Pair_Data[])
{
  if (ev_call_table[ACI_EATT_CLT_READ_BY_TYPE_RESP_EVENT] != NULL) {
      aci_eatt_clt_read_by_type_resp_event_type ptr = (aci_eatt_clt_read_by_type_resp_event_type)ev_call_table[ACI_EATT_CLT_READ_BY_TYPE_RESP_EVENT];
      ptr (Connection_Handle,CID,Handle_Value_Pair_Length,Data_Length,Handle_Value_Pair_Data);
  }
}
typedef void (* aci_eatt_clt_read_resp_event_type)(uint16_t Connection_Handle,
                                           uint16_t CID,
                                           uint16_t Event_Data_Length,
                                           uint8_t Attribute_Value[]);
/**
...
void aci_eatt_clt_read_resp_event(uint16_t Connection_Handle,
                                  ...
                                  uint8_t Attribute_Value[])
{
  if (ev_call_table[ACI_EATT_CLT_READ_RESP_EVENT] != NULL) {
      aci_eatt_clt_read_resp_event_type ptr = (aci_eatt_clt_read_resp_event_type)ev_call_table[ACI_EATT_CLT_READ_RESP_EVENT];
      ptr (Connection_Handle,CID,Event_Data_Length,Attribute_Value);
  }
}
typedef void (* aci_eatt_clt_read_blob_resp_event_type)(uint16_t Connection_Handle,
                                                uint16_t CID,
                                                uint16_t Event_Data_Length,
                                                uint8_t Attribute_Value[]);
/**
...
void aci_eatt_clt_read_blob_resp_event(uint16_t Connection_Handle,
                                       ...
                                       uint8_t Attribute_Value[])
{
  if (ev_call_table[ACI_EATT_CLT_READ_BLOB_RESP_EVENT] != NULL) {
      aci_eatt_clt_read_blob_resp_event_type ptr = (aci_eatt_clt_read_blob_resp_event_type)ev_call_table[ACI_EATT_CLT_READ_BLOB_RESP_EVENT];
      ptr (Connection_Handle,CID,Event_Data_Length,Attribute_Value);
  }
}
typedef void (* aci_eatt_clt_read_multiple_resp_event_type)(uint16_t Connection_Handle,
                                                    uint16_t CID,
                                                    uint16_t Event_Data_Length,
                                                    uint8_t Set_Of_Values[]);
/**
...
void aci_eatt_clt_read_multiple_resp_event(uint16_t Connection_Handle,
                                           ...
                                           uint8_t Set_Of_Values[])
{
  if (ev_call_table[ACI_EATT_CLT_READ_MULTIPLE_RESP_EVENT] != NULL) {
      aci_eatt_clt_read_multiple_resp_event_type ptr = (aci_eatt_clt_read_multiple_resp_event_type)ev_call_table[ACI_EATT_CLT_READ_MULTIPLE_RESP_EVENT];
      ptr (Connection_Handle,CID,Event_Data_Length,Set_Of_Values);
  }
}
typedef void (* aci_eatt_clt_read_by_group_type_resp_event_type)(uint16_t Connection_Handle,
                                                         uint16_t CID,
                                                         uint8_t Attribute_Data_Length,
                                                         uint16_t Data_Length,
                                                         uint8_t Attribute_Data_List[]);
...
void aci_eatt_clt_read_by_group_type_resp_event(uint16_t Connection_Handle,
                                                ...
                                                uint8_t Attribute_Data_List[])
{
  if (ev_call_table[ACI_EATT_CLT_READ_BY_GROUP_TYPE_RESP_EVENT] != NULL) {
      aci_eatt_clt_read_by_group_type_resp_event_type ptr = (aci_eatt_clt_read_by_group_type_resp_event_type)ev_call_table[ACI_EATT_CLT_READ_BY_GROUP_TYPE_RESP_EVENT];
      ptr (Connection_Handle,CID,Attribute_Data_Length,Data_Length,Attribute_Data_List);
  }
}
typedef void (* aci_eatt_clt_prepare_write_resp_event_type)(uint16_t Connection_Handle,
                                                    uint16_t CID,
                                                    uint16_t Attribute_Handle,
                                                    uint16_t Offset,
                                                    uint16_t Part_Attribute_Value_Length,
...
void aci_eatt_clt_prepare_write_resp_event(uint16_t Connection_Handle,
                                           ...
                                           uint8_t Part_Attribute_Value[])
{
  if (ev_call_table[ACI_EATT_CLT_PREPARE_WRITE_RESP_EVENT] != NULL) {
      aci_eatt_clt_prepare_write_resp_event_type ptr = (aci_eatt_clt_prepare_write_resp_event_type)ev_call_table[ACI_EATT_CLT_PREPARE_WRITE_RESP_EVENT];
      ptr (Connection_Handle,CID,Attribute_Handle,Offset,Part_Attribute_Value_Length,Part_Attribute_Value);
  }
}
typedef void (* aci_eatt_clt_exec_write_resp_event_type)(uint16_t Connection_Handle,
                                                 uint16_t CID);
/**
 * @brief This event is generated in response to an ATT Execute Write Request,
...
void aci_eatt_clt_exec_write_resp_event(uint16_t Connection_Handle,
                                        uint16_t CID)
{
  if (ev_call_table[ACI_EATT_CLT_EXEC_WRITE_RESP_EVENT] != NULL) {
      aci_eatt_clt_exec_write_resp_event_type ptr = (aci_eatt_clt_exec_write_resp_event_type)ev_call_table[ACI_EATT_CLT_EXEC_WRITE_RESP_EVENT];
      ptr (Connection_Handle,CID);
  }
}
typedef void (* aci_gatt_eatt_clt_indication_event_type)(uint16_t Connection_Handle,
                                                 uint16_t CID,
                                                 uint16_t Attribute_Handle,
                                                 uint16_t Attribute_Value_Length,
                                                 uint8_t Attribute_Value[]);
...
void aci_gatt_eatt_clt_indication_event(uint16_t Connection_Handle,
                                        ...
                                        uint8_t Attribute_Value[])
{
  if (ev_call_table[ACI_GATT_EATT_CLT_INDICATION_EVENT] != NULL) {
      aci_gatt_eatt_clt_indication_event_type ptr = (aci_gatt_eatt_clt_indication_event_type)ev_call_table[ACI_GATT_EATT_CLT_INDICATION_EVENT];
      ptr (Connection_Handle,CID,Attribute_Handle,Attribute_Value_Length,Attribute_Value);
  }
}
typedef void (* aci_gatt_eatt_clt_notification_event_type)(uint16_t Connection_Handle,
                                                   uint16_t CID,
                                                   uint16_t Attribute_Handle,
                                                   uint16_t Attribute_Value_Length,
                                                   uint8_t Attribute_Value[]);
...
void aci_gatt_eatt_clt_notification_event(uint16_t Connection_Handle,
                                          ...
                                          uint8_t Attribute_Value[])
{
  if (ev_call_table[ACI_GATT_EATT_CLT_NOTIFICATION_EVENT] != NULL) {
      aci_gatt_eatt_clt_notification_event_type ptr = (aci_gatt_eatt_clt_notification_event_type)ev_call_table[ACI_GATT_EATT_CLT_NOTIFICATION_EVENT];
      ptr (Connection_Handle,CID,Attribute_Handle,Attribute_Value_Length,Attribute_Value);
  }
}
typedef void (* aci_gatt_eatt_clt_proc_complete_event_type)(uint16_t Connection_Handle,
                                                    uint16_t CID,
                                                    uint8_t Error_Code);
/**
...
void aci_gatt_eatt_clt_proc_complete_event(uint16_t Connection_Handle,
                                           ...
                                           uint8_t Error_Code)
{
  if (ev_call_table[ACI_GATT_EATT_CLT_PROC_COMPLETE_EVENT] != NULL) {
      aci_gatt_eatt_clt_proc_complete_event_type ptr = (aci_gatt_eatt_clt_proc_complete_event_type)ev_call_table[ACI_GATT_EATT_CLT_PROC_COMPLETE_EVENT];
      ptr (Connection_Handle,CID,Error_Code);
  }
}
typedef void (* aci_gatt_eatt_clt_error_resp_event_type)(uint16_t Connection_Handle,
                                                 uint16_t CID,
                                                 uint8_t Req_Opcode,
                                                 uint16_t Attribute_Handle,
                                                 uint8_t Error_Code);
...
void aci_gatt_eatt_clt_error_resp_event(uint16_t Connection_Handle,
                                        ...
                                        uint8_t Error_Code)
{
  if (ev_call_table[ACI_GATT_EATT_CLT_ERROR_RESP_EVENT] != NULL) {
      aci_gatt_eatt_clt_error_resp_event_type ptr = (aci_gatt_eatt_clt_error_resp_event_type)ev_call_table[ACI_GATT_EATT_CLT_ERROR_RESP_EVENT];
      ptr (Connection_Handle,CID,Req_Opcode,Attribute_Handle,Error_Code);
  }
}
typedef void (* aci_gatt_eatt_clt_disc_read_char_by_uuid_resp_event_type)(uint16_t Connection_Handle,
                                                                  uint16_t CID,
                                                                  uint16_t Attribute_Handle,
                                                                  uint8_t Attribute_Value_Length,
                                                                  uint8_t Attribute_Value[]);
...
void aci_gatt_eatt_clt_disc_read_char_by_uuid_resp_event(uint16_t Connection_Handle,
                                                         ...
                                                         uint8_t Attribute_Value[])
{
  if (ev_call_table[ACI_GATT_EATT_CLT_DISC_READ_CHAR_BY_UUID_RESP_EVENT] != NULL) {
      aci_gatt_eatt_clt_disc_read_char_by_uuid_resp_event_type ptr = (aci_gatt_eatt_clt_disc_read_char_by_uuid_resp_event_type)ev_call_table[ACI_GATT_EATT_CLT_DISC_READ_CHAR_BY_UUID_RESP_EVENT];
      ptr (Connection_Handle,CID,Attribute_Handle,Attribute_Value_Length,Attribute_Value);
  }
}
typedef void (* aci_gatt_eatt_srv_confirmation_event_type)(uint16_t Connection_Handle,
                                                   uint16_t CID);
/**
 * @brief This event is generated when the client has sent the confirmation to a
...
void aci_gatt_eatt_srv_confirmation_event(uint16_t Connection_Handle,
                                          uint16_t CID)
{
  if (ev_call_table[ACI_GATT_EATT_SRV_CONFIRMATION_EVENT] != NULL) {
      aci_gatt_eatt_srv_confirmation_event_type ptr = (aci_gatt_eatt_srv_confirmation_event_type)ev_call_table[ACI_GATT_EATT_SRV_CONFIRMATION_EVENT];
      ptr (Connection_Handle,CID);
  }
}
typedef void (* aci_eatt_srv_exec_write_req_event_type)(uint16_t Connection_Handle,
                                                uint16_t CID,
                                                uint8_t Flags);
/**
 * @brief This event is generated when an execute write request is received from
...
void aci_eatt_srv_exec_write_req_event(uint16_t Connection_Handle,
                                       ...
                                       uint8_t Flags)
{
  if (ev_call_table[ACI_EATT_SRV_EXEC_WRITE_REQ_EVENT] != NULL) {
      aci_eatt_srv_exec_write_req_event_type ptr = (aci_eatt_srv_exec_write_req_event_type)ev_call_table[ACI_EATT_SRV_EXEC_WRITE_REQ_EVENT];
      ptr (Connection_Handle,CID,Flags);
  }
}
typedef void (* aci_eatt_clt_read_multiple_var_len_resp_event_type)(uint16_t Connection_Handle,
                                                            uint16_t CID,
                                                            uint16_t Event_Data_Length,
                                                            uint8_t Set_Of_Values[]);
/**
...
void aci_eatt_clt_read_multiple_var_len_resp_event(uint16_t Connection_Handle,
                                                   ...
                                                   uint8_t Set_Of_Values[])
{
  if (ev_call_table[ACI_EATT_CLT_READ_MULTIPLE_VAR_LEN_RESP_EVENT] != NULL) {
      aci_eatt_clt_read_multiple_var_len_resp_event_type ptr = (aci_eatt_clt_read_multiple_var_len_resp_event_type)ev_call_table[ACI_EATT_CLT_READ_MULTIPLE_VAR_LEN_RESP_EVENT];
      ptr (Connection_Handle,CID,Event_Data_Length,Set_Of_Values);
  }
}
typedef void (* aci_gatt_eatt_srv_read_event_type)(uint16_t Connection_Handle,
                                           uint16_t CID,
                                           uint16_t Attribute_Handle,
                                           uint16_t Data_Offset);
/**
...
void aci_gatt_eatt_srv_read_event(uint16_t Connection_Handle,
                                  ...
                                  uint16_t Data_Offset)
{
  if (ev_call_table[ACI_GATT_EATT_SRV_READ_EVENT] != NULL) {
      aci_gatt_eatt_srv_read_event_type ptr = (aci_gatt_eatt_srv_read_event_type)ev_call_table[ACI_GATT_EATT_SRV_READ_EVENT];
      ptr (Connection_Handle,CID,Attribute_Handle,Data_Offset);
  }
}
typedef void (* aci_gatt_eatt_srv_write_event_type)(uint16_t Connection_Handle,
                                            uint16_t CID,
                                            uint8_t Resp_Needed,
                                            uint16_t Attribute_Handle,
                                            uint16_t Data_Length,
...
void aci_gatt_eatt_srv_write_event(uint16_t Connection_Handle,
                                   ...
                                   uint8_t Data[])
{
  if (ev_call_table[ACI_GATT_EATT_SRV_WRITE_EVENT] != NULL) {
      aci_gatt_eatt_srv_write_event_type ptr = (aci_gatt_eatt_srv_write_event_type)ev_call_table[ACI_GATT_EATT_SRV_WRITE_EVENT];
      ptr (Connection_Handle,CID,Resp_Needed,Attribute_Handle,Data_Length,Data);
  }
}
typedef void (* aci_eatt_srv_prepare_write_req_event_type)(uint16_t Connection_Handle,
                                                   uint16_t CID,
                                                   uint16_t Attribute_Handle,
                                                   uint16_t Data_Offset,
                                                   uint16_t Data_Length,
...
void aci_eatt_srv_prepare_write_req_event(uint16_t Connection_Handle,
                                          ...
                                          uint8_t Data[])
{
  if (ev_call_table[ACI_EATT_SRV_PREPARE_WRITE_REQ_EVENT] != NULL) {
      aci_eatt_srv_prepare_write_req_event_type ptr = (aci_eatt_srv_prepare_write_req_event_type)ev_call_table[ACI_EATT_SRV_PREPARE_WRITE_REQ_EVENT];
      ptr (Connection_Handle,CID,Attribute_Handle,Data_Offset,Data_Length,Data);
  }
}
...
typedef void (* hci_le_cis_established_event_type)(uint8_t Status,
                                           uint16_t Connection_Handle,
...
void hci_le_cis_established_event(uint8_t Status,
                                  ...
                                  uint16_t ISO_Interval)
{
  if (ev_call_table[HCI_LE_CIS_ESTABLISHED_EVENT] != NULL) {
      hci_le_cis_established_event_type ptr = (hci_le_cis_established_event_type)ev_call_table[HCI_LE_CIS_ESTABLISHED_EVENT];
      ptr (Status,Connection_Handle,CIG_Sync_Delay,CIS_Sync_Delay,Transport_Latency_C_To_P,Transport_Latency_P_To_C,PHY_C_To_P,PHY_P_To_C,NSE,BN_C_To_P,BN_P_To_C,FT_C_To_P,FT_P_To_C,Max_PDU_C_To_P,Max_PDU_P_To_C,ISO_Interval);
  }
}
typedef void (* hci_le_cis_request_event_type)(uint16_t ACL_Connection_Handle,
...
void hci_le_cis_request_event(uint16_t ACL_Connection_Handle,
                              ...
                              uint8_t CIS_ID)
{
  if (ev_call_table[HCI_LE_CIS_REQUEST_EVENT] != NULL) {
      hci_le_cis_request_event_type ptr = (hci_le_cis_request_event_type)ev_call_table[HCI_LE_CIS_REQUEST_EVENT];
      ptr (ACL_Connection_Handle,CIS_Connection_Handle,CIG_ID,CIS_ID);
  }
}
typedef void (* hci_le_create_big_complete_event_type)(uint8_t Status,
                                               uint8_t BIG_Handle,
...
void hci_le_create_big_complete_event(uint8_t Status,
                                      ...
                                      Connection_Handle_t Connection_Handle[])
{
  if (ev_call_table[HCI_LE_CREATE_BIG_COMPLETE_EVENT] != NULL) {
      hci_le_create_big_complete_event_type ptr = (hci_le_create_big_complete_event_type)ev_call_table[HCI_LE_CREATE_BIG_COMPLETE_EVENT];
      ptr (Status,BIG_Handle,BIG_Sync_Delay,Transport_Latency_BIG,PHY,NSE,BN,PTO,IRC,Max_PDU,ISO_Interval,Num_BIS,Connection_Handle);
  }
}
typedef void (* hci_le_terminate_big_complete_event_type)(uint8_t BIG_Handle,
                                                  uint8_t Reason);
/**
...
void hci_le_terminate_big_complete_event(uint8_t BIG_Handle,
                                         uint8_t Reason)
{
  if (ev_call_table[HCI_LE_TERMINATE_BIG_COMPLETE_EVENT] != NULL) {
      hci_le_terminate_big_complete_event_type ptr = (hci_le_terminate_big_complete_event_type)ev_call_table[HCI_LE_TERMINATE_BIG_COMPLETE_EVENT];
      ptr (BIG_Handle,Reason);
  }
}
typedef void (* hci_le_big_sync_established_event_type)(uint8_t Status,
                                                uint8_t BIG_Handle,
                                                uint8_t Transport_Latency_BIG[3],
                                                uint8_t NSE,
                                                uint8_t BN,
...
void hci_le_big_sync_established_event(uint8_t Status,
                                       ...
                                       Connection_Handle_t Connection_Handle[])
{
  if (ev_call_table[HCI_LE_BIG_SYNC_ESTABLISHED_EVENT] != NULL) {
      hci_le_big_sync_established_event_type ptr = (hci_le_big_sync_established_event_type)ev_call_table[HCI_LE_BIG_SYNC_ESTABLISHED_EVENT];
      ptr (Status,BIG_Handle,Transport_Latency_BIG,NSE,BN,PTO,IRC,Max_PDU,ISO_Interval,Num_BIS,Connection_Handle);
  }
}
typedef void (* hci_le_big_sync_lost_event_type)(uint8_t BIG_Handle,
                                         uint8_t Reason);
/**
...
void hci_le_big_sync_lost_event(uint8_t BIG_Handle,
                                uint8_t Reason)
{
  if (ev_call_table[HCI_LE_BIG_SYNC_LOST_EVENT] != NULL) {
      hci_le_big_sync_lost_event_type ptr = (hci_le_big_sync_lost_event_type)ev_call_table[HCI_LE_BIG_SYNC_LOST_EVENT];
      ptr (BIG_Handle,Reason);
  }
}
typedef void (* hci_le_request_peer_sca_complete_type)(uint8_t Status,
                                               uint16_t Connection_Handle,
 ...
 * @retval None
 */
void hci_le_request_peer_sca_complete(uint8_t Status,
                                      uint16_t Connection_Handle,
                                      uint8_t Peer_Clock_Accuracy)
{
  if (ev_call_table[HCI_LE_REQUEST_PEER_SCA_COMPLETE] != NULL) {
      hci_le_request_peer_sca_complete_type ptr = (hci_le_request_peer_sca_complete_type)ev_call_table[HCI_LE_REQUEST_PEER_SCA_COMPLETE];
      ptr (Status,Connection_Handle,Peer_Clock_Accuracy);
  }
}
...
typedef void (* hci_le_biginfo_advertising_report_event_type)(uint16_t Sync_Handle,
                                                      uint8_t Num_BIS,
                                                      uint8_t NSE,
                                                      uint16_t ISO_Interval,
                                                      uint8_t BN,
...
void hci_le_biginfo_advertising_report_event(uint16_t Sync_Handle,
                                             ...
                                             uint8_t Encryption)
{
  if (ev_call_table[HCI_LE_BIGINFO_ADVERTISING_REPORT_EVENT] != NULL) {
      hci_le_biginfo_advertising_report_event_type ptr = (hci_le_biginfo_advertising_report_event_type)ev_call_table[HCI_LE_BIGINFO_ADVERTISING_REPORT_EVENT];
      ptr (Sync_Handle,Num_BIS,NSE,ISO_Interval,BN,PTO,IRC,Max_PDU,SDU_Interval,Max_SDU,PHY,Framing,Encryption);
  }
}
typedef void (* hci_le_subrate_change_event_type)(uint8_t Status,
                                          uint16_t Connection_Handle,
...
void hci_le_subrate_change_event(uint8_t Status,
                                 ...
                                 uint16_t Supervision_Timeout)
{
  if (ev_call_table[HCI_LE_SUBRATE_CHANGE_EVENT] != NULL) {
      hci_le_subrate_change_event_type ptr = (hci_le_subrate_change_event_type)ev_call_table[HCI_LE_SUBRATE_CHANGE_EVENT];
      ptr (Status,Connection_Handle,Subrate_Factor,Peripheral_Latency,Continuation_Number,Supervision_Timeout);
  }
}
void * ev_call_table[110] = {
  NULL,
  NULL,
  NULL,
  NULL,
